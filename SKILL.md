...
```json
{
  ...
  "total_docs": 300,
  "created_by_book": {
    "456": {"created": 120, "local_created": 120, "multi_category_copies": 15},
    "789": {"created": 50, "local_created": 50, "multi_category_copies": 0}
  },
  "skipped": 15,
...
- `total_docs`：源库文档总数
- `created_by_book`：按知识库 ID 分组的计数器，支持多目标库切换后保留历史数据
  - `{book_id}.created`：该库累计创建文档数（含多目录副本）
...
**迁移脚本默认后台运行，禁止前台阻塞主线程。**

```bash
# ✅ 正确：nohup 扔后台 + tail -1 确认启动
nohup python3 scripts/migrate.py progress/xxx.json > logs/migrate.log 2>&1 &
tail -1 logs/migrate.log  # 确认"📦 续传"出现即启动成功

# ❌ 错误：直接前台跑
python3 scripts/migrate.py progress/xxx.json

# ❌ 错误：等日志时用 sleep && tail 阻塞
sleep 20 && tail -30 logs/migrate.log
```

**查进度**：读进度 JSON 的 `last_offset` / `created_by_book` / `skipped` / `failed` 字段，**秒级返回，不阻塞**。

**防重复**：脚本自带 PID 锁文件 `progress/{book_id}_{name}.json.lock`，同进度文件不会重复启动。
...
> 容量初始计数只调一次 API，迁移过程中**本地累加** `local_created`。切换目标库时重新获取新目标库的初始文档数。详见步骤 3c。
...
3. 取 `format` 字段：
   ...
   - **lake** → 正常处理。`?raw=1` 返回近似 markdown 格式，可与 markdown 同等清洗迁移
