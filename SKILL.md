...
用户说「将《xxx》内容整理到《yyy》」后，先检查 `yuque-config.json` 是否包含 `llm` 字段。没有则引导用户填写：
...
| 进度 | `utils/yuque-migration/progress/{book_id}_{旧库名}.json` |
...
5. **LLM 清洗+分类（合并一次调用）**：
   ...
   **长文档截断**：单次喂入上限 **20000 字符**。超过则截取前 20000 字符送入 LLM，由 LLM 判断内容完整性，选择一个自然的段落/章节边界作为截断点，输出截断后的正文 + 末尾分类标记 `<!-- CATEGORIES: ["分类1", "分类2"] -->`。分类按内容多选，宁可少分类不误分类。
