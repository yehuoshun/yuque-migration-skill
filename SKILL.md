...
```json
{
  ...
  "rate_limit": {"remaining": 1234, "last_checked": "..."},
  "initial_count": 4500,
  "local_created": 300,
  "target_history": [
    {"book_id": 111, "book_name": "目标库A", "doc_count": 5000},
    {"book_id": 456, "book_name": "目标库B", "doc_count": 150}
  ]
}
```
| API 文档 | `skills/yuque-ai/references/api_reference.md` |
| 批量迁移脚本 | `skills/yuque-migration/scripts/migrate_batch.py` |

> 脚本说明：migrate_batch.py 通过内容采样检测二进制（非ASCII+控制字符 > 25%），不依赖标题匹配。源/目标库 ID 需在脚本内修改。
...
若 >= 5000 → 暂停，提示用户「目标库已达上限 5000 篇，请提供新目标库」
...
2. 空文档跳过。先 `GET /repos/{book_id}/docs/{doc_id}?raw=1` 读取原文，取 `format` 字段：
   ...
   - 空 body（去除空白后为空）→ 记入空文档跳过
3. **格式清洗**：丢给 LLM 清理广告/免责条款/水评论/HTML残留，保留有技术价值的评论和内部链接。发现附件引用（图片/文件链接）标注到 `docs_with_attachments` 清单。
...
```
4. **去重**：先用 `GET /search?q={标题}&type=doc&scope={目标库namespace}` 搜标题 → 有匹配则 GET 该文档正文，逐级比对（200 字 → 500 字 → 全文），任意一级不同即视为不同文档；完全相同跳过；标题相同内容不同加 `(重复标题-N)`，N 为目标库已有同名最大编号+1
5. **大文档**：>50000 字（约 200KB，不统计代码块内字数）→ 按内容结构拆分，标题加 `(1/N)` 后缀<br>拆分优先级：`##` 标题 → `###` 标题 → 段落边界（空行）<br>铁律：不断在段内、不跨代码块/表格切、代码块跟随最近的标题整块带走、每份 ≤50000 字
6. `POST /repos/{book_id}/docs` 创建文档，收集 `doc_id`
...
若 current_count >= 5000 → 暂停迁移，提示用户：
  「⚠️ 目标库《xxx》已达上限 5000 篇，已迁移 N 篇。请提供新的目标库名称，或确认原目标库有空间后说『继续整理』」
```
...
3. 在进度文件中追加历史目标库记录（用于最终汇报）：
   ```json
   "target_history": [
     {"book_id": 456, "book_name": "目标库A", "doc_count": 5000},
