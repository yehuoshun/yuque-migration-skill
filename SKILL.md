...
如果迁移已完成但目录未建立（如临时脚本迁移后遗症），单独补建目录：

```bash
python3 scripts/migrate.py --toc-only <进度文件路径>
```

此时跳过文档迁移，直接执行 LLM 分类 + 建目录 + 挂文档。

**降级兼容**：`classify_and_build_toc` 优先取 `created_docs_content`（含标题+正文摘要，分类更准）；若无此字段但有 `created_doc_mapping`，自动从目标库 API 拉取标题补齐数据后再分类。
