...
```json
{
  ...
  "local_created": 300,
  "target_history": [
    {"book_id": 111, "book_name": "目标库A", "doc_count": 4800},
...
> 铁律：步骤 3c 不是可选的，迁移脚本必须实现 TOC 建目录。漏了 = bug。
...
若 >= 4800 → 暂停，提示用户「目标库已达切换阈值 4800 篇，请提供新目标库」
...
3. **二进制检测**：内容采样前 100 字符，若非 ASCII + 控制字符比例 > 25% 则判定为二进制文件 → 跳过（不记入 failed，直接跳过）
...
若 current_count >= 4800 → 暂停迁移，提示用户：
  「⚠️ 目标库《xxx》已达切换阈值 4800 篇，已迁移 N 篇。请提供新的目标库名称，或确认原目标库有空间后说『继续整理』」
```
...
3. 在进度文件中追加历史目标库记录（用于最终汇报）：
   ```json
   "target_history": [
     {"book_id": 456, "book_name": "目标库A", "doc_count": 4800},
