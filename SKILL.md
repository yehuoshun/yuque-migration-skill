...
以 **钉钉 Markdown 格式** 输出报告，可直接推送到钉钉机器人。
...
- **大文档**：W 篇（已拆分）
- **含附件文档**：V 篇（附件无法迁移，请手动处理）
- **Lake 无损搬运**：L 篇（lake 格式原样搬运）
- **失败**：U 篇
...
失败、孤儿文档较多时，可追加 `### 失败清单` / `### 孤儿清单` 小节逐条列出。

推送方式：
```
POST https://oapi.dingtalk.com/robot/send?access_token={token}
Body: { "msgtype": "markdown", "markdown": { "title": "迁移完成", "text": "..." } }
```
...
- **单迁移任务**：同时只允许一个迁移任务运行（多个任务会触发语雀 API 高并发限制）
- 同时处理的文档不超过 5 篇
